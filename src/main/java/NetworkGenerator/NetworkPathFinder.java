...
public class NetworkPathFinder { //gets shortest path from network to network
  private HashMap<Server, Double> pathCost;
  private HashMap<Server, Server> prevNode;

  public AuxiliaryNetwork shortestPathsByCost(Network n, Request r,
      CostFunction fun) {//use djikstra to get shortest paths by utilization cost. New Link takes the minimum bandwidth in shortest path.
...
/*				HashMap<Integer, ArrayList<Link>> destMap = (allShortestPaths.containsKey(dest.getId())) ?
    ...
    return new AuxiliaryNetwork(n.getServers(), n.getLinks(), pathCosts, pathDelays, allShortestPaths, r);
  }

  private void insertSort(ArrayList<Server> queue, Server s) {
    if (queue.isEmpty()) {
      queue.add(s);
    }
