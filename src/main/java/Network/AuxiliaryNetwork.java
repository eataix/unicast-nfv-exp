...
public class AuxiliaryNetwork extends Network{
	HashMap<Integer, HashMap<Integer, ArrayList<Link>>> allShortestPaths;
	double [][] pathCosts;
	double [][] pathDelays;
	public ArrayList<HashSet<Server>> serviceLayers; 
	Request request;
	Server src;
	Server dst;
	ArrayList<Server> auxServers; 
	ArrayList<Link> auxLinks;
	...
	public void generateNetwork(boolean offline){ //create network with auxServers and auxLinks
		...
		auxServers.add(src); auxServers.add(dst);
